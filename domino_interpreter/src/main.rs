...
enum Instuction {
    ADDI, ADD, JUMP, BEQ, LI, INPUT, OUTPUT, EXIT
}
    ...
    match args.get(1) {
        Some(file) => {
            if file.ends_with(".domino") {
                ...
                return Err(io::Error::new(io::ErrorKind::Other, "file must end with .domino"))
            }
        }
        None => return Err(io::Error::new(io::ErrorKind::Other, "use format ./domino <filepath>"))
    }
        ...
        match line {
            Some(l) => {
                ...
                match parse_instruction(instuction)? {
                    ADD => {add(l, &mut registers)?;}
                    ADDI => {addi(l, &mut registers)?;}
                    JUMP => {jump(l,&mut curr_line)?; continue;}
                    LI => {load_immediate(l, &mut registers)?;}
                    BEQ => {branch_if_equal(l, &mut registers, &mut curr_line)?; continue;}
                    INPUT => {input(&mut registers)?;}
                    OUTPUT => {output(&mut registers);}
                    EXIT => {break}
                }
            }
            None => return Err(io::Error::new(io::ErrorKind::Other, "Program exited unintentionally"))
        }
        curr_line += 1;
...
fn branch_if_equal(instruction: &str, registers: &mut [i32; 4], ptr: &mut usize) -> Result<(), io::Error> {
    let r1_c = instruction.chars().nth(1).unwrap();
    let r2_c = instruction.chars().nth(2).unwrap();
    let r1 = parse_register(r1_c)?;
    let r2 = parse_register(r2_c)?;
...
fn parse_imm(s: &str, size: u8) -> Result<i32, io::Error> {
    ...
    for c in s.chars() {
        let bits = match c { //this match statement was lovely to code
            '🀱' => "",
            '🀲' => "0",
            '🀳' => "00",
            '🀴' => "000",
    ...
    if first_bit == "1" { //if the int is negative we need to inverse the string
        bit_string = bit_string.chars()
        .map(|c| if c == '0' { '1' } else { '0' })
        .collect();
    }
    
    bit_string = bit_string[..1].to_string() + "0".repeat(32-bit_string.len()).as_str() + &bit_string[1..];
